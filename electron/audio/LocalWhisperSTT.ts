...
export class LocalWhisperSTT extends EventEmitter {
                ...
                try {
                    ...
                    try {
                        ...
                        if (cleanedTranscript && cleanedTranscript.length > 0) {
                            console.log(`[LocalWhisperSTT] Transcript: "${cleanedTranscript.substring(0, 50)}${cleanedTranscript.length > 50 ? '...' : ''}"`);
    ...
    /**
     * Remove hallucinations and non-speech artifacts from the transcript
     */
    private cleanTranscript(text: string): string {
        if (!text) return "";
        let cleaned = text.trim();

        // 1. Remove non-speech patterns (music, noise, etc.)
        for (const pattern of NON_SPEECH_TRANSCRIPT_PATTERNS) {
            if (pattern.test(cleaned)) return "";
        }

        // 2. Remove common hallucinations (especially on silence or low-quality audio)
        ...
        for (const h of hallucinations) {
            if (h.test(cleaned)) return "";
        }

        // 3. Remove repeating patterns (e.g. "Thank you. Thank you. Thank you.")
        // if the string is just one short sentence repeated 3+ times, it's likely a hallucination
        const sentences = cleaned.split(/(?<=[.!?])\s+/);
        if (sentences.length >= 3) {
            const first = sentences[0].toLowerCase().trim();
            if (first.length < 20 && sentences.every(s => s.toLowerCase().trim() === first)) {
                return "";
            }
        }
