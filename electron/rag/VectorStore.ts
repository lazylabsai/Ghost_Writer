...
export class VectorStore {
    ...
    /**
     * Migration check:
     * If existing embeddings are 768d (Gemini), clear them so they can be re-embedded as 384d (Local)
     */
    private validateEmbeddingDimensions(): void {
        const sample = this.db.prepare('SELECT embedding FROM chunks WHERE embedding IS NOT NULL LIMIT 1').get() as any;
        if (!sample || !sample.embedding) return;

        const embedding = this.blobToEmbedding(sample.embedding);
        if (embedding.length !== 384) {
            console.log(`[VectorStore] Dimension mismatch detected (${embedding.length} != 384). Clearing legacy embeddings...`);
            this.db.prepare('UPDATE chunks SET embedding = NULL').run();
            this.db.prepare('UPDATE chunk_summaries SET embedding = NULL').run();
            // Clear queue errors/status so they re-embed
            this.db.prepare("UPDATE embedding_queue SET status = 'pending', retry_count = 0 WHERE status = 'completed'").run();
        }
    }
