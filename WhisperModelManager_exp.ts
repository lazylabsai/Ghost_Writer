 ...
 *   Γö£ΓöÇΓöÇ bin/
 *   Γöé   ΓööΓöÇΓöÇ main.exe (whisper.cpp binary)
 *   ΓööΓöÇΓöÇ models/
 *       ΓööΓöÇΓöÇ ggml-small.bin (whisper model)
 */
...
export class WhisperModelManager extends EventEmitter {
    ...
    private async downloadBinary(cuda: boolean): Promise<boolean> {
        ...
        try {
            ...
            if (cuda) {
                console.log(`[WhisperModelManager] CUDA support: ${this.hasCUDASupport() ? 'YES Γ£à' : 'NO Γ¥î'}`);
            }
            return true;
        } catch (err) {
    ...
    private async buildFromSource(sourceUrl: string): Promise<boolean> {
        ...
        try {
            ...
            console.log('[WhisperModelManager] Metal GPU acceleration: ENABLED Γ£à');
